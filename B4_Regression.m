...
for j=1:6
    ...
    for cvIdx = 1:CVO.NumTestSets
        ...
        model = fitnlm( xTr ,yTr , modelfun , beta0 );
        predictedYTe= predict(model,xTe);
        predictedYExpOneVariable(j,teIdx)=predictedYTe;
    end
    measureExpOneVariable=computeMeasures(predictedYExpOneVariable(j,:),YData(:,currentY),3);
    resultsExpOneVariable(j,:)=measureExpOneVariable.array;
end
save('.\results\B4\regression.mat');